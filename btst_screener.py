...
<style>
  ...
  *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .ts{{font-family:var(--mono);text-align:right;flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:2px}}
  .ts-time{{font-size:clamp(1.05rem,2.2vw,1.25rem);font-weight:700;color:var(--blue);white-space:nowrap;display:flex;align-items:center;gap:6px;line-height:1.2}}
  .ts-date{{font-size:clamp(.82rem,1.5vw,.96rem);color:#d0dce8;font-weight:600;white-space:nowrap}}
  .ts-label{{font-size:clamp(.66rem,1.2vw,.76rem);color:#7888a0;letter-spacing:.5px}}
  .tz-tag{{display:inline-block;background:rgba(96,212,255,.15);color:var(--blue);border:1px solid rgba(96,212,255,.4);border-radius:4px;font-family:var(--mono);font-size:.7rem;padding:2px 7px;letter-spacing:1px;vertical-align:middle;font-weight:700}}
...
<div class="header">
  <div class="header-top">
    ...
    <div class="ts">
      <div class="ts-time">
        <span id="live-clock">{now_ist.strftime('%I:%M:%S %p')}</span>
        <span class="tz-tag">IST</span>
      </div>
      <div class="ts-date">{now_ist.strftime('%d %b %Y')}</div>
      <div class="ts-label">Last generated · Auto-report</div>
    </div>
  </div>
